...
fn configurate() {
    ...
    println!("Le bot se connectera à votre compte et likera des publications dans des hashtags ciblés.");
    ...
    io::stdin().read_line(&mut hashtags)
        .expect("Failed to read line");
    println!("Répondre \"oui\" pour utiliser Firefox.");
    io::stdin().read_line(&mut browser)
        .expect("Failed to read line");
    ...
    let mut config_file = File::create("config.txt").expect("Impossible de créer le fichier config.txt.");
    config_file.write_all(json::stringify(json::object!(
        "username" => username,
        "password" => password,
        "hashtags" => hashtags,
        "browser" => browser
    )).as_bytes()).expect("Impossible d'écrire dans le fichier config.txt.");
...
fn main() {
    ...
    loop {
        ...
        println!("[3] : Lancer le bot sur tous les hashtags rensignés dans le fichier de configuration");
        ...
        match answer {
            ...
            2 => {
                ...
                io::stdin().read_line(&mut hashtag)
                    .expect("Failed to read line");
...
fn launch_bot(username: &str, password: &str, hashtags: Vec<String>, browser: Browser) {
    ...
    tab.navigate("https://www.instagram.com/accounts/login/?source=auth_switcher").unwrap();
    thread::sleep(Duration::from_millis(5000));
    let mut username_block = tab.find(Selector::Css, "input[name=\"username\"]").unwrap().unwrap();
    let mut password_block = tab.find(Selector::Css, "input[name=\"password\"]").unwrap().unwrap();
    let mut submit_block = tab.find(Selector::Css, "button[type=\"submit\"]").unwrap().unwrap();
    username_block.type_text(username).unwrap();
    password_block.type_text(password).unwrap();
    submit_block.click().unwrap();

    while let None = tab.find(Selector::XPath, "/html/body/div[3]/div/div/div[3]/button[2]").unwrap() {
        thread::sleep(Duration::from_millis(100));
    }

    let mut notif = tab.find(Selector::XPath, "/html/body/div[3]/div/div/div[3]/button[2]").unwrap().unwrap();
    notif.click().unwrap();
    ...
    for hashtag in hashtags {
        ...
        for y in 1..999 {
            for x in 1..4 {
                ...
                let mut image = tab.find(Selector::XPath, &xpath).unwrap().unwrap();
                image.click().unwrap();
                thread::sleep(Duration::from_millis(4000));
                
                if let Some(mut heart) = tab.find(Selector::XPath, "/html/body/div[3]/div[2]/div/article/div[2]/section[1]/span[1]/button/span").unwrap() {
                    heart.click().unwrap();
                    thread::sleep(Duration::from_millis(1000));
                }

                if let Some(mut close) = tab.find(Selector::XPath, "/html/body/div[3]/button[1]").unwrap() {
                    close.click().unwrap();
